...
import trex.common.ComplexParameter;
...
import trex.common.EventTemplateAttr;
import trex.common.EventTemplateStaticAttr;
import trex.common.Negation;
import trex.common.OpTree;
import trex.common.OpValueReference;
...
import trex.common.StaticValueReference;
import trex.packets.*;
...
public class Marshaller {
    ...
    public static byte[] getByteArray(PubPkt pkt) throws IOException {
        byte[] dest = new byte[4 + getNumBytes(PktType.PUB_PKT) + getNumBytes(pkt)];
        encode(pkt, dest, 0);
        return dest;
    }

    public static byte[] getByteArray(RulePkt pkt, EngineType type) throws IOException {
        byte[] dest = new byte[4 + getNumBytes(PktType.RULE_PKT) + getNumBytes(pkt)];
        encode(pkt, dest, 0, type);
        return dest;
    }

    public static byte[] getByteArray(SubPkt pkt) throws IOException {
        byte[] dest = new byte[4 + getNumBytes(PktType.SUB_PKT) + getNumBytes(pkt)];
        encode(pkt, dest, 0);
        return dest;
    }

    public static byte[] getByteArray(UnPubPkt pkt) throws IOException {
        byte[] dest = new byte[4 + getNumBytes(PktType.UNPUB_PKT) + getNumBytes(pkt)];
        encode(pkt, dest, 0);
        return dest;
    }

    public static byte[] getByteArray(AdvPkt pkt) throws IOException {
    ...
    protected static int getNumBytes(RulePkt pkt) throws IOException {
        int size = 0;
        //size += getNumBytes(EngineType.CPU);
        /**
         * RuleID#
         */
        size += getNumBytes(pkt.getRuleId());
        size += getNumBytes(pkt.getPredicates());
        size += getNumBytes(pkt.getComplexParameters());
    ...
    protected static int encode(RulePkt source, byte[] dest, int startIndex, EngineType eType) throws IOException {
        ...
        startIndex = encode(source.getRuleId(), dest, startIndex);
        startIndex = encode(source.getPredicates(), dest, startIndex);
        startIndex = encode(source.getComplexParameters(), dest, startIndex);
    ...
    protected static int getNumBytes(PubPkt pkt) throws IOException {
        int size = 0;
    ...
    protected static int encode(PubPkt source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int getNumBytes(SubPkt pkt) throws IOException {
        int size = 0;
        size += getNumBytes(pkt.getEventType());
    ...
    protected static int getNumBytes(UnPubPkt pkt) throws IOException {
        int size = 0;
        size += getNumBytes(pkt.getEventType());
        size += getNumBytes(pkt.getAttributes());
        return size;
    ...
    protected static int encode(SubPkt source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int encode(UnPubPkt source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(PktType.UNPUB_PKT, dest, startIndex);
        startIndex = encode(getNumBytes(source), dest, startIndex);
        startIndex = encode(source.getEventType(), dest, startIndex);
        startIndex = encode(source.getAttributes(), dest, startIndex);
        return startIndex;
    ...
    protected static int getNumBytes(AdvPkt pkt) throws IOException {
        int size = 0;
        size += getNumBytes(pkt.getAdvertisements());
        return size;
    }

    protected static int encode(AdvPkt source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int getNumBytes(Constraint constraint) throws IOException {
        int size = 0;
        ...
        else if (constraint.getValType() == ValType.GEOMETRY) size += getNumBytes(constraint.getGeometryVal());
        return size;
    }

    protected static int encode(Constraint source, byte[] dest, int startIndex) throws IOException {
        ...
        else if (source.getValType() == ValType.GEOMETRY) startIndex = encode(source.getGeometryVal(), dest, startIndex);
        return startIndex;
    }

    protected static int getNumBytes(Attribute att) throws IOException {
        int size = 0;
        ...
        else if (att.getValType() == ValType.GEOMETRY) size += getNumBytes(att.getGeometryVal());
        return size;
    }

    protected static int encode(Attribute source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getName(), dest, startIndex);
        startIndex = encode(source.getValType(), dest, startIndex);
        if (source.getValType() == ValType.INT) startIndex = encode(source.getIntVal(), dest, startIndex);
        else if (source.getValType() == ValType.FLOAT) startIndex = encode(source.getFloatVal(), dest, startIndex);
    ...
    protected static int getNumBytes(EventPredicate pred) throws IOException {
        int size = 0;
    ...
    protected static int encode(EventPredicate source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getEventType(), dest, startIndex);
        startIndex = encode(source.getConstraints(), dest, startIndex);
    ...
    protected static int getNumBytes(ComplexParameter par) throws IOException {
        int size = 0;
    ...
    protected static int encode(ComplexParameter source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int getNumBytes(Negation neg) throws IOException {
        int size = 0;
    ...
    protected static int encode(Negation source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getEventType(), dest, startIndex);
        startIndex = encode(source.getConstraints(), dest, startIndex);
    ...
    protected static int getNumBytes(TAggregate agg) throws IOException {
        int size = 0;
    ...
    protected static int encode(TAggregate source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getEventType(), dest, startIndex);
        startIndex = encode(source.getConstraints(), dest, startIndex);
        startIndex = encode(source.getLowerId(), dest, startIndex);
        startIndex = encode(source.getLowerTime(), dest, startIndex);
    ...
    protected static int getNumBytes(EventTemplate template) throws IOException {
        int size = 0;
    ...
    protected static int encode(EventTemplate source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getEventType(), dest, startIndex);
        startIndex = encode(source.getAttributes(), dest, startIndex);
    ...
    protected static int getNumBytes(EventTemplateAttr templateAttr) throws IOException {
        int size = 0;
    ...
    protected static int getNumBytes(EventTemplateStaticAttr templateAttr) throws IOException {
        int size = 0;
        size += getNumBytes(templateAttr.getAttr());
        return size;
    }

    protected static int encode(EventTemplateAttr source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getName(), dest, startIndex);
        startIndex = encode(source.getValue(), dest, startIndex);
        return startIndex;
    }

    protected static int encode(EventTemplateStaticAttr source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getAttr(), dest, startIndex);
        return startIndex;
    }

    protected static int getNumBytes(OpTree tree) throws IOException {
        int size = 0;
    ...
    protected static int encode(OpTree source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int getNumBytes(OpValueReference valRef) throws IOException {
        int size = 0;
    ...
    protected static int getNumBytes(StaticValueReference valRef, int size) throws IOException {
        ...
        if (valRef.getType() == ValType.GEOMETRY) size += getNumBytes(valRef.getGeometryVal());
        return size;
    }
    ...
    protected static int encode(OpValueReference source, byte[] dest, int startIndex) throws IOException {
    ...
    protected static int encode(StaticValueReference source, byte[] dest, int startIndex) throws IOException {
        startIndex = encode(source.getType(), dest, startIndex);
        ...
        if (source.getType() == ValType.GEOMETRY) startIndex = encode(source.getGeometryVal(), dest, startIndex);
        return startIndex;
    }
    ...
    protected static int getNumBytes(Geometry geometry) throws IOException {
        // GeometryWriter.writeGeometry
        // GeometryPrinter.getGeometryString
        // return GeometryPrinter.getGeometryString(geometry).length() + 4; // One int (4 bytes) more to encode the length of the string
        //return GeometryWriter.writeGeometry(geometry).length() + 4; // One int (4 bytes) more to encode the length of the string
        return geometry.toText().length() + 4; // One int (4 bytes) more to encode the length of the string
    }

    protected static int encode(Geometry geometry, byte[] dest, int startIndex) throws IOException {
        //String source = GeometryWriter.writeGeometry(geometry);
        String source = geometry.toText();
        //String source = GeometryPrinter.getGeometryString(geometry);
        int length = source.length();
        startIndex = encode(length, dest, startIndex);
        for (int i = 0; i < length; i++) {
            dest[startIndex + i] = (byte) source.charAt(i);
    ...
    protected static int encode(ConstraintOp source, byte[] dest, int startIndex) {
        ...
        else if (source == ConstraintOp.NE) dest[startIndex++] = 3;
    ...
    protected static int encode(ValType source, byte[] dest, int startIndex) {
        ...
        else if (source == ValType.GEOMETRY) dest[startIndex++] = 4;
        return startIndex;
    }
    ...
    protected static int encode(PktType source, byte[] dest, int startIndex) {
        ...
        else if (source == PktType.UNPUB_PKT) dest[startIndex++] = 5;
        return startIndex;
    }

    protected static int getNumBytes(Collection<? extends Object> collection) throws IOException {
    ...
    protected static int encode(Collection<? extends Object> source, byte[] dest, int startIndex) throws IOException {
        ...
        for (Object el : source) {
            ...
            else if (el instanceof EventTemplateStaticAttr)
                startIndex = encode((EventTemplateStaticAttr) el, dest, startIndex);
        }
        return startIndex;
    }

    protected static int getNumBytes(Map<Integer, ? extends Object> map) throws IOException {
        int size = 4;    // One int (4 bytes) to encode the length of the collection
    ...
    protected static int encode(Map<Integer, ? extends Object> source, byte[] dest, int startIndex) throws IOException {
        int size = source.size();
        startIndex = encode(size, dest, startIndex);
    ...
    public static byte[] getByteArray(UnSubPkt pkt) throws IOException {
    ...
    protected static int getSize(SubPkt subPkt) throws IOException {
        return BYTENUM_PKTTYPE + BYTENUM_PKTLENGTH + Marshaller.getNumBytes(subPkt);
    }

    protected static int getSize(UnPubPkt unPubPkt) throws IOException {
        return BYTENUM_PKTTYPE + BYTENUM_PKTLENGTH + Marshaller.getNumBytes(unPubPkt);
    }

    protected static int getNumBytes(UnSubPkt pkt) throws IOException {
     ...
     */
    public static byte[] marshalRule(TRexPkt pkt, EngineType eType) throws IOException {
     ...
     * Flattens the given packet to an array of bytes.
     */
    public static byte[] marshal(TRexPkt pkt) throws IOException {
        ...
        else if (pkt instanceof UnPubPkt) return getByteArray((UnPubPkt) pkt);
