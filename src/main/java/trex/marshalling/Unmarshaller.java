...
public class Unmarshaller {
    ...
    public static PubPkt decodePubPkt(byte[] source) throws IOException {
        IndexWrapper index = new IndexWrapper();
        return decodePubPkt(source, index);
    }

    public static RulePkt decodeRulePkt(byte[] source) throws IOException {
        IndexWrapper index = new IndexWrapper();
        return decodeRulePkt(source, index);
    }

    public static SubPkt decodeSubPkt(byte[] source) throws IOException {
        IndexWrapper index = new IndexWrapper();
        return decodeSubPkt(source, index);
    }

    public static UnPubPkt decodePubRegisterPkt(byte[] source) throws IOException {
        IndexWrapper index = new IndexWrapper();
    ...
    protected static RulePkt decodeRulePkt(byte[] source, IndexWrapper index) throws IOException {
        /**
         * RuleId#
         */
        Integer ruleId = decodeInt(source, index);
        HashMap<Integer, EventPredicate> predicates = decodeEventPredicateMap(source, index);
        HashMap<Integer, ComplexParameter> complexParameters = decodeParameterMap(source, index);
        ...
        for (Integer key : complexParameters.keySet()) {
            pkt.addComplexParameter(complexParameters.get(key));
        }
    ...
    protected static PubPkt decodePubPkt(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static SubPkt decodeSubPkt(byte[] source, IndexWrapper index) throws IOException {
        int eventType = decodeInt(source, index);
    ...
    protected static UnPubPkt decodePubRegisterPkt(byte[] source, IndexWrapper index) throws IOException {
        int eventType = decodeInt(source, index);
        Collection<Attribute> attributes = decodeAttributeCollection(source, index);
    ...
    protected static Constraint decodeConstraint(byte[] source, IndexWrapper index) throws IOException {
        ...
        if (type == ValType.INT) {
            ...
            Geometry val = decodeGeometry(source, index);
            return new Constraint(name, op, val);
        }
    }

    protected static Attribute decodeAttribute(byte[] source, IndexWrapper index) throws IOException {
        String name = decodeString(source, index);
        ValType type = decodeValType(source, index);
        if (type == ValType.INT) {
            int val = decodeInt(source, index);
    ...
    protected static EventPredicate decodeEventPredicate(byte[] source, IndexWrapper index) throws IOException {
        int eventType = decodeInt(source, index);
        Collection<Constraint> constraints = decodeConstraintCollection(source, index);
    ...
    protected static ComplexParameter decodeParameter(byte[] source, IndexWrapper index) throws IOException {
        //TODO
        ConstraintOp op = decodeConstraintOp(source, index);
    ...
    protected static Negation decodeNegation(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static TAggregate decodeAggregate(byte[] source, IndexWrapper index) throws IOException {
        int evType = decodeInt(source, index);
        Collection<Constraint> constraints = decodeConstraintCollection(source, index);
        int lowerId = decodeInt(source, index);
        long lowerTime = decodeLong(source, index);
    ...
    protected static EventTemplate decodeEventTemplate(byte[] source, IndexWrapper index) throws IOException {
        int eventType = decodeInt(source, index);
    ...
    protected static EventTemplateAttr decodeEventTemplateAttr(byte[] source, IndexWrapper index) throws IOException {
        String name = decodeString(source, index);
        OpTree value = decodeOpTree(source, index);
        return new EventTemplateAttr(name, value);
    }

    protected static OpTree decodeOpTree(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static OpValueReference decodeValueReference(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static StaticValueReference decodeStaticValueReference(byte[] source, IndexWrapper index) throws IOException {
        ValType type = decodeValType(source, index);
        if (type == ValType.INT)
        ...
        else if (type==ValType.STRING) return new StaticValueReference(
                    decodeString(source, index));
        else return new StaticValueReference(
                    decodeGeometry(source, index));
    }
    ...
    protected static Geometry decodeGeometry(byte[] source, IndexWrapper index) throws IOException {
        int length = decodeInt(source, index);
        char[] charArray = new char[length];
        for (int i = 0; i < length; i++) {
            charArray[i] = (char) source[index.get() + i];
        ...
        try {
            return rdr.read(returnValue);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return new GeometryFactory().createGeometry(null);
    }
    ...
    protected static ValType decodeValType(byte[] source, IndexWrapper index) {
        ...
        else if (source[pos] == 3) return ValType.STRING;
        else return ValType.GEOMETRY;
    }

    protected static HashMap<Integer, EventPredicate> decodeEventPredicateMap(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static HashMap<Integer, ComplexParameter> decodeParameterMap(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static HashMap<Integer, Negation> decodeNegationMap(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static HashMap<Integer, TAggregate> decodeAggregateMap(byte[] source, IndexWrapper index) throws IOException {
    ...
    protected static Collection<Constraint> decodeConstraintCollection(byte[] source, IndexWrapper index) throws IOException {
        int length = decodeInt(source, index);
    ...
    protected static Collection<Attribute> decodeAttributeCollection(byte[] source, IndexWrapper index) throws IOException {
        int length = decodeInt(source, index);
    ...
    protected static Collection<EventTemplateAttr> decodeEventTemplateAttrCollection(byte[] source, IndexWrapper index) throws IOException {
        int length = decodeInt(source, index);
    ...
    public static PubPkt decodePubPkt(byte[] source, int startIndex) throws IOException {
        IndexWrapper index = new IndexWrapper();
    ...
    public static RulePkt decodeRulePkt(byte[] source, int startIndex) throws IOException {
        IndexWrapper index = new IndexWrapper();
        index.inc(startIndex);
    ...
    public static SubPkt decodeSubPkt(byte[] source, int startIndex) throws IOException {
        IndexWrapper index = new IndexWrapper();
        index.inc(startIndex);
    ...
    public static UnPubPkt decodePubRegisterPkt(byte[] source, int startIndex) throws IOException {
        IndexWrapper index = new IndexWrapper();
        index.inc(startIndex);
    ...
    public static UnSubPkt decodeUnSubPkt(byte[] source, int startIndex) throws IOException {
     ...
     */
    public static TRexPkt unmarshal(byte[] buffer, MutableInt offset) throws IOException {
     ...
     */
    public static TRexPkt unmarshal(byte[] buffer) throws IOException {
