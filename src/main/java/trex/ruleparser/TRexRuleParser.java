...
import trex.common.ParametersMap;
...
import trex.ruleparser.TESLAParser.Aggregate_atomContext;
import trex.ruleparser.TESLAParser.Attr_constraintContext;
import trex.ruleparser.TESLAParser.Attr_declarationContext;
import trex.ruleparser.TESLAParser.Attr_definitionContext;
import trex.ruleparser.TESLAParser.Attr_parameterContext;
import trex.ruleparser.TESLAParser.ExprContext;
import trex.ruleparser.TESLAParser.Neg_betweenContext;
import trex.ruleparser.TESLAParser.Neg_one_referenceContext;
import trex.ruleparser.TESLAParser.Packet_referenceContext;
import trex.ruleparser.TESLAParser.Param_atomContext;
import trex.ruleparser.TESLAParser.PredicateContext;
import trex.ruleparser.TESLAParser.StaticAttr_definitionContext;
...
public class TRexRuleParser extends TESLABaseListener {
	 ...
	 * Event attributes, even for different event types, with the same name must have the same ValType (INT, FLOAT, BOOL, STRING or GEOMETRY)
	...
	private ConstraintOp getConstrOp(String source) {
		...
		else if (source.equals("INTERSECT")) return ConstraintOp.INTERSECT;
		else if (source.equals("CONTAINS")) return ConstraintOp.CONTAINS;
		else if (source.equals("DISTANCE")) return ConstraintOp.DISTANCE;
		else if (source.equals("INTERIORPOINT")) return ConstraintOp.INTERIORPOINT;
		else return null;
	}
	...
	private ValType getValType(String vtype) {
		...
		else if (vtype.equals("GEOMETRY")) return ValType.STRING;
		else return null;
	}
	...
	private Constraint getConstraint(Attr_constraintContext cons) {
		...
		else if (cons.static_reference().GEOMETRY_VAL() != null) {
			String val = cons.static_reference().GEOMETRY_VAL().getText();
			c = new Constraint(cons.ATTR_NAME().getText(), op, val);
		}
		return c;
	}
	...
	private OpTree recursivelyNavigateExpression(ExprContext expr, OpTree tree, ValType valType) {
		if (expr.param_atom() != null) {
			...
			if (ctx.static_reference() != null) {
				...
				else if (ctx.static_reference().GEOMETRY_VAL() != null) {
					Geometry val = null;
					WKTReader rdr = new WKTReader();

					try {
						val = rdr.read(ctx.static_reference().STRING_VAL().getText());
					} catch (ParseException e) {
						e.printStackTrace();
					}
					value = new StaticValueReference(val);
					vtype = ValType.GEOMETRY;
				}
	...
	@Override public void enterAggregate_atom(@NotNull Aggregate_atomContext ctx) {
	...
	@Override public void enterDefinitions(@NotNull TESLAParser.DefinitionsContext ctx) {
		...
		for (StaticAttr_definitionContext sdef : statics) {
			...
			else if (vtype == ValType.GEOMETRY) {
				Geometry val = null;
				WKTReader rdr = new WKTReader();
				try {
					val = rdr.read(sdef.static_reference().STRING_VAL().getText());
				} catch (Exception e) {
					e.printStackTrace();
				}
				sattr = new EventTemplateStaticAttr(sdef.ATTR_NAME().getText(), val);
