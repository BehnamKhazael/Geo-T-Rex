...
WHERE 	   : 'Where';
CONSUMING  : 'Consuming';
VALTYPE    : 'String' | 'Int' | 'Float' | 'Bool' | 'Quality' | 'Geometry' ;
SEL_POLICY : 'Each' | 'Last' | 'First' ;
AGGR_FUN   : 'AVG' | 'SUM' | 'MAX' | 'MIN' | 'COUNT' | 'CENTER' ;
OPERATOR   : '=' | '>' | '<' | '>=' | '<=' | '!=' | '&' | '|' | 'INTERSECT' | 'CONTAINS' | 'DISTANCE' | 'INTERIORPOINT';
LOGICAL_OPERATOR   : 'And' | 'Or' ;
BINOP_MUL  : '*' | '/';
BINOP_ADD  : '+' | '-' ;
SPATIAL_OP : 'INTERSECTION' | 'CENTROID' | 'DISTANCEFROM' ;
INT_VAL    : ('0' .. '9')+;
FLOAT_VAL  : ('0' .. '9')+ '.' ('0' .. '9')+ ;
BOOL_VAL   : 'False' | 'True' ;
STRING_VAL :   '"'
...
static_reference : (INT_VAL | FLOAT_VAL | STRING_VAL | BOOL_VAL | GEOMETRY_VAL);
...
expr: expr BINOP_MUL expr | expr BINOP_ADD expr | expr SPATIAL_OP expr | '(' expr ')' | (param_atom | aggregate_atom);
...
simple_event : ASSIGN event_declarations ending_rule;
complex_event :  DEFINE ce_definition FROM pattern (WHERE definitions)? (CONSUMING consuming)? ending_rule;
simple_event_definition : simple_event;
complex_event_definition : ASSIGN event_declarations complex_event;